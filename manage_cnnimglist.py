...
from parts.controller.SRDriver import Driver
...
class Drive(BaseCommand):
    def parse_args(self, args):
        ...
        parser.add_argument('--imglist_path', help='The path of the img')
    ...
    def run(self, args):
        ...
        self.drive(cfg=cfg,model_path=args.model,imglist_Path=args.imglist_path)

    def drive(self,cfg, model_path=None,imglist_Path=None):

        #Initialize car
        V = Vehicle()

        cam = ImageListCamera(imglist_Path+'/*.jpg')
        V.add(cam, outputs=['cam/image_array','img_path'], threaded=True)

        #sViewer 
        sviewer=sViewr()
        V.add(sviewer,inputs=['img_path','pilot/angle'],threaded=False)
