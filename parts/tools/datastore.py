...
class OriginalWriter:
    ...
    def run(self, *args):
        '''
        API function needed to use as a SenseRover part.
...
class Tub(object):
    ...
    def get_train_gen(self, X_keys, Y_keys, batch_size=32, seq_len=4, record_transform=None, df=None):
        ...
        # while True:
            ...
            # X = [batch[k] for k in X_keys]
            # Y = [batch[k] for k in Y_keys]
            # list, with 2 arrays included
            # the first dimension gives the angle (shape: [batchsize, 15] as onehot)
            # the second dimension gives the throttle (shape: batchsize)
            # # print('\n\n')
            # # print('y:',Y)
            # # print('\n\n')
...
class TubGroup(Tub):
    def __init__(self, tub_paths_arg):
        tub_paths = file.expand_path_arg(tub_paths_arg)
