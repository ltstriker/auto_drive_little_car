...
class Joystick():
    ...
    def __init__(self, dev_fn='/dev/input/js0'):
        ...
        self.button_names = {
            ...
            0x130 : 'a', # X button in f170
            0x131 : 'b', # A button in f170
            0x132 : 'c', # B button in f170
            0x133 : 'x', # Y button in f170
            0x134 : 'y', # LB button in f170
            0x135 : 'z', # RB button in f170
            0x136 : 'tl', # LT button in f170 (note: the output of pressing once is 1+0 instead of just 1)
            0x137 : 'tr', # RT button in f170
            0x138 : 'tl2',
            0x139 : 'tr2',
            0x13a : 'select',
            0x13b : 'start',
...
class JoystickController(object):
    ...
    def __init__(self, poll_delay=0.0,
                 ...
                 # steering_axis='x',
                 # throttle_axis='rz',
                 steering_axis = 'z', # left axis
                 throttle_axis = 'y', # right axis
                 steering_scale=1.0,
                 throttle_scale=-1.0,
        ...
        while self.running:
            ...
            if button == 'b' and button_state == 1:
                ...
                if self.auto_record_on_throttle:
                    ...
                    self.recording = False
                    print('Data record has stopped!')
                else:
                    self.recording = True
                    print('Data record has started!')
            ...
            if button == 'z' and button_state == 1:
                ...
                '''
                self.max_throttle = round(min(1.0, self.max_throttle + 0.1), 2)
            ...
            if button == 'y' and button_state == 1:
                ...
                '''
                self.max_throttle = round(max(0.0, self.max_throttle - 0.1), 2)
                if self.constant_throttle:
                    self.throttle = self.max_throttle
                    self.on_throttle_changes()
